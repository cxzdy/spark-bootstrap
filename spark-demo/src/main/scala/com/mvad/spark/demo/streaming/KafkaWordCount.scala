 ...
 *      com.mvad.spark.com.mvad.spark.demo.streaming.KafkaWordCount zk1dg,zk2dg,zk3dg:2181/ \
  ...
  def main(args: Array[String]) {
    ...
    lines.foreachRDD((rdd: RDD[String], time: Time) => {
      ...
      val wordsDataFrame = rdd.map(base64Decode)
        .map(w => DSPLogInfo(w.getShowInfo.getAdvertiserId, w.getShowInfo.getFee)).toDF()
