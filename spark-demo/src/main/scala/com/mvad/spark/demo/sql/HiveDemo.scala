...
object HiveDemo {
  ...
  def main(args: Array[String]) {
    ...
    val sc = new SparkContext(new SparkConf().setAppName("SparkSQL:[com.mvad.spark.demo][SparkSQLUsingHive]"))
    ...
    val sql =
      """
        ...
        |where date='2015-09-01' AND solution_entity = 'galileo' AND ad_container != 'APP'
    ...
    val count = d_clickvalue.filter("date='2015-09-01'").groupBy("hour").count()
    count.show()
